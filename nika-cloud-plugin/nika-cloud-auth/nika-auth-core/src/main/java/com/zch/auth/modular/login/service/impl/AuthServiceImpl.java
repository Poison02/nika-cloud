package com.zch.auth.modular.login.service.impl;
...
import com.zch.auth.modular.login.enums.AuthExceptionEnum;
...
import com.zch.common.util.CommonCryptogramUtil;
...
public class AuthServiceImpl implements AuthService {
    ...
    private static final String NIKA_SYS_DEFAULT_CAPTCHA_OPEN_KEY = "NIKA_SYS_DEFAULT_CAPTCHA_OPEN_KEY";
    ...
    public AuthPicValidCodeResult getPicCaptcha(String type) {
        ...
        // 将请求号作为key，验证码的值作为value存入redis，用于校验，5分钟有效
    ...
    public String getPhoneValidCode(AuthGetPhoneValidCodeParam authGetPhoneValidCodeParam, String type) {
        ...
        签名不传则使用系统默认配置的签名，支持传入多个参数，示例：{"name": "张三", "number": "15035****79"}
        */
        // devSmsApi.sendSmsAliyun(phone, null, "验证码模板号", JSONUtil.toJsonStr(JSONUtil.createObj().set("validCode", phoneValidCode)));
        /*
        使用腾讯云执行发送验证码，将验证码作为短信内容的参数变量放入
        sdkAppId和签名不传则使用系统默认配置的sdkAppId和签名，支持传入多个参数，逗号拼接，示例："张三,15038****76,进行中"
         */
        devSmsApi.sendSmsTencent("sdkAppId", phone, "签名", "模板编码", phoneValidCode);
        // 将请求号作为key，验证码的值作为value放入redis中，用于叫遗憾，5分钟有效
    ...
    public String doLogin(AuthAccountPasswordLoginParam authAccountPasswordLoginParam, String type) {
        ...
        // 校验验证码
        String defaultCaptchaOpen = devConfigApi.getValueByKey(NIKA_SYS_DEFAULT_CAPTCHA_OPEN_KEY);
    ...
    /**
     * 执行B端登录
     * @param saBaseLoginUser
     * @param device
     * @return
     */
    public String execLoginB(SaBaseLoginUser saBaseLoginUser, String device) {
        // 校验状态
        ...
        List<String> roleCodeList = roleList.stream()
                ...
                .collect(Collectors.toList());
        // 角色id和用户id集合
        List<String> userAndRoleIdList =
                CollectionUtil.unionAll(roleIdList, CollectionUtil.newArrayList(saBaseLoginUser.getId()));
        // 获取按钮码
        ...
        // 缓存用户信息，此处使用TokenSession为了指定时间内误操作则自动下线
    ...
    /**
     * 执行C端登录
     * @param saBaseClientLoginUser
     * @param device
     * @return
     */
    public String execLoginC(SaBaseClientLoginUser saBaseClientLoginUser, String device) {
// 校验状态
